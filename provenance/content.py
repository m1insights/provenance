...
class Candidate:
    ...
    #: could be sweep-shaped -- see ``sweep_signals``.
...
def score(appraisal: Appraisal, paper: Paper | None, *, posted: set[str]) -> tuple[float, list[str]]:
    ...
    if motion == "hold":
        ...
        # A dose-response curve gives the sweep motion a
        # path to travel.
        # 
        # 
        total += 15.0
...
def candidates(
    ...
    *,
    ...
    ``sweepable`` narrows to papers carrying dose-response signals,
    as a filter rather than a separate pool.
    """
